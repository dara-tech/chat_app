...
export async function POST(
    ...
    try {
        ...
        if (isGroup && (!members || members.length < 2 || !name)) {
            return new NextResponse('Invalid group chat data. Name and at least 2 members required.', { status: 400 });
        }

        // Handle group conversation creation
        if (isGroup) {
            // Verify all members exist
            const memberIds = members.map((member: { value: string }) => member.value);
            ...
            if (existingMembers.length !== memberIds.length) {
                return new NextResponse('One or more members not found', { status: 400 });
            }
        ...
        if (!userId) {
            return new NextResponse('User ID is required for direct conversations', { status: 400 });
        }
